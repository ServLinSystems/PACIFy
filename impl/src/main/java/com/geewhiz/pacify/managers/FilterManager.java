...
public class FilterManager {
	...
	public LinkedHashSet<Defect> doFilter() {
		...
		for (Object entry : pMarker.getFilesAndArchivesAndXMLFiles()) {
			...
			} else if (entry instanceof PXml) {
				defects.addAll(filterPXml((PXml) entry));
			} else {
	...
	private LinkedHashSet<Defect> filterPXml(PXml pXml) {
	...
	private File extractFile(PArchive pArchive, PFile pFile) {
		try {
			...
			// Existence is checked before, so we should not get this exception
			// here
			throw new RuntimeException(e);
		}
	}
